...
// Login/Signup with Google OAuth (with optional redirect)
...
// Send password reset email (you chose forgot-password.html)
export async function resetPassword(email) {
  const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: window.location.origin + '/docs/forgot-password.html',
  });
  if (error) throw error;
  return data;
}
