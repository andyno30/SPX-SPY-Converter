...
`game-app/` owns engine, adapters, tests, credentials and isolated Supabase migrations. `game-app/content/` owns version-controlled research and future authored definitions. `game-app/game-docs/research/` is the research bible. All game material is now physically under `game-app/`. No existing auth client is imported. The intended URL prefix is `/game/`, including assets and authentication callbacks; a dedicated Vercel project is the selected host. See [deployment preparation](arpia-deployment.md) for current verification and final domain-routing constraints.
