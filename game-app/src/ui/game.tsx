...
export default function Game({data,manifest}:{data:unknown;manifest:unknown}){
  ...
  return <div className="arpia-page">
    <div className="page-top"><span>Arpia · English reconstruction</span><span>Local playtest · Episodes 0–2</span></div>
...
function Play({session,assets,leave,store}:{session:AdventureSession;assets:AssetResolver;leave:()=>void;store:BrowserAdventureStore}){
  ...
  useEffect(()=>{let cancelled=false;void import('./world').then(({createWorld})=>{if(!cancelled&&host.current)world.current=createWorld(host.current,session,assets,setUIError);}).catch(e=>setUIError('Unable to open the game world: '+e.message));return()=>{cancelled=true;world.current?.destroy();world.current=null;};},[session,assets]);
  ...
  const backup=()=>{const blob=new Blob([store.export(session.state)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='arpia-adventure.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
  return <main className="play-layout">
    <div className="game-wrap" ref={shell} style={{height:640*scale}}>
      <div className="game-frame" style={{transform:`scale(${scale})`} as CSSProperties}>
        ...
        {!s.battle&&<>
          <div className="quest-note"><span>Current mission</span><strong>{quest?.name??'A little time at school'}</strong><p>{step?.label??'Opening chapter complete. Visit teachers, train your pets, or explore the school.'}</p>{hint&&<button disabled={session.busy||!!s.dialogue} onClick={()=>approach(hint.id)}>{step?.mapId===map.id?'Find ':'Go via '}{hint.name} →</button>}</div>
