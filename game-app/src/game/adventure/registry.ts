...
export function validateCampaign(input:unknown,manifest?:AssetManifest,sourceIds?:Set<string>):Campaign {
  ...
  for(const q of c.quests){
    ...
    for(const s of q.steps){
      ...
      if(s.type==='TYPE_PHRASE'&&!s.phrase)fail('missing puzzle phrase');
      effects(s.effects);
    }effects(q.rewards);
  }
  ...
  for(const service of c.services){ref(actors,service.actorId,'service NPC');ref(maps,service.mapId,'service map');service.itemIds.forEach(id=>ref(items,id,'shop item'));if(service.spellId)ref(spells,service.spellId,'lesson');if(service.toMapId)ref(maps,service.toMapId,'teleport');}
  return c;
}
