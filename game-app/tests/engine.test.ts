...
test('content numbers and provenance validate; dependency cycles and missing IDs fail',()=>{
  const c=checkContent();assert.equal(c.main.length,103);assert.equal(c.free.length,37);assert.equal(c.runtimeQuestCount,9);assert.equal(c.runtimeMapCount,44);
